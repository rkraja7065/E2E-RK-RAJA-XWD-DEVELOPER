...
def stop_task(task_id):
    ...
    if task_id in st.session_state.stop_events:
        ...
        add_log(f"🛑 Stopping task: {task_id}", "warning")
        return True
    return False

def stop_all_tasks():
    """STOP ALL TASKS FUNCTION"""
    if not st.session_state.tasks:
        add_log("ℹ️ No active tasks to stop", "info")
        return False
    
    task_count = len(st.session_state.tasks)
    add_log(f"🛑 Stopping ALL {task_count} tasks...", "warning")
    ...
    add_log(f"✅ All {task_count} tasks stopped successfully!", "success")
    return True
...
def txt_file_selection():
    """Display TXT file buttons"""
    st.markdown("### 📄 SELECT GALLI FILE")
...
# =========================================================
# STOP ALL TASKS BUTTON SECTION
# =========================================================
def stop_all_tasks_section():
    """Display STOP ALL TASKS button"""
    st.markdown("---")
    
    # Big Red STOP ALL Button
    st.markdown("### 💥 EMERGENCY STOP")
    
    if st.session_state.tasks:
        active_tasks = sum(1 for t in st.session_state.tasks.values() if t.get("status") == "Running")
        ...
        if active_tasks > 0:
            ...
            with col2:
                if st.button(
                    ...
                    if stop_all_tasks():
                        st.success(f"✅ Stopped all {active_tasks} tasks!")
                        st.rerun()
                    else:
                        st.error("❌ Failed to stop tasks")
            
            st.info(f"⚠️ **Warning:** This will stop ALL {active_tasks} running tasks immediately!")
        else:
            st.info("ℹ️ No active tasks running")
    else:
        st.info("ℹ️ No tasks created yet")
...
def main_panel():
    ...
    with st.form("main_form"):
        ...
        if not st.session_state.selected_txt_file:
            ...
            st.success(f"✅ Ready: {st.session_state.selected_txt_file}")
    ...
    # STOP ALL TASKS Button Section
    stop_all_tasks_section()
    
    # Individual task stop section
    st.markdown("---")
    st.markdown("### 🛑 STOP INDIVIDUAL TASK")
        ...
        for task_id, task_info in st.session_state.tasks.items():
            ...
            with col3:
                if st.button("❌", key=f"remove_{task_id}"):
                    ...
                    if task_id in st.session_state.stop_events:
                        del st.session_state.stop_events[task_id]
                    add_log(f"🗑️ Removed task: {task_id}", "warning")
                    st.rerun()
...
🎯 NEW STOP ALL FEATURES ADDED:

1. BIG RED STOP ALL BUTTON:

```python
# Emergency Stop Button
if st.button("⛔ STOP ALL TASKS NOW", type="primary"):
    stop_all_tasks()
```

2. STOP ALL TASKS FUNCTION:

```python
def stop_all_tasks():
    """Stop all running tasks at once"""
    for task_id in list(st.session_state.stop_events.keys()):
        st.session_state.stop_events[task_id].set()
    ...
    add_log(f"✅ All tasks stopped!", "success")
